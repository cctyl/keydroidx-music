 ...
 * 音乐 App 侧图标码点补充。
 *
 * 用途：`keydroidx-core` 的 [io.github.cctyl.nokia.common.ui.KeydroidxIcons] 是生态共享字典，
...
object MusicIcons {
    ...
    /** 评论气泡（Material `chat_bubble` 实心）。缺字形时回退 `KeydroidxIcons.ICON_SUBTITLES`。 */
    const val COMMENT = "\uE0CA"

    /** 点赞（Material `thumb_up`）。缺字形时回退 `KeydroidxIcons.ICON_FAVORITE`。 */
    const val THUMB_UP = "\uE8DC"
}
