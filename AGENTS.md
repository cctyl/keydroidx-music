...
- **KeydroidX Launcher (桌面端 / 中枢)**：
  ...
  - 负责待机桌面、通知栏与按键机全局设置。
  - 通过 `KeydroidxKeyProvider` 向外提供物理按键映射、主题、字体及缩放配置。
- **keydroidx-core (通用 SDK 核心库)**：
  - 路径：`../keydroidx-core`
  - 生态客户端通用基础库（包含 Model, Client, UI, Dialog）。
  - 负责四级降级按键解析、跨进程配置热同步与复古 UI 规范。
- **keydroidx-music (本项目 - 独立应用)**：
  - 纯物理按键驱动的轻量音乐播放器。
  - 依赖 `keydroidx-core` 并继承 `KeydroidxBaseActivity`，开箱即用获得生态能力。

### 1.2 核心机制与协同工作流
1. **零二次配置**：
   - 用户在 KeydroidX 桌面配好物理按键或切换主题/字体后，音乐播放器通过 ContentProvider 即时自动换肤并生效，无需用户进入音乐 App 重新配键。
2. **四级平滑降级**：
   - 优先同步正式版桌面 Provider 配置，其次 Debug 版桌面 Provider；若设备未安装桌面，则降级为本应用本地独立配置；若无本地配置则回退为 Android 系统标准键值。
3. **开箱即用的复古 UI**：
   - 列表页与播放详情页遵循 240×320 设计基准的响应式原生 DP 布局（不做运行时缩放），自动应用生态点阵字体（ArkPixel / FusionPixel）与 MaterialIcons 矢量图标。
   - 弹窗统一采用 SDK 内置的 `KeydroidxOptionsDialog`（选项菜单）与 `KeydroidxConfirmDialog`（确认弹窗）。
...
- **宿主 `MainActivity`**（继承 `keydroidx-core` 的 `KeydroidxBaseActivity`）：
...
SDK 已经将 240×320 设计基准的响应式原生 DP 布局（无运行时缩放）、按键解析分发、点阵字体整树应用、主题动态联动和焦点防出界滚动固化在基类中。**宿主应用严禁直接继承 Android 原生 `Activity` 或 `Fragment`。**

```
┌───────────────────────────────────────────────────────────┐
│               KeydroidxBaseActivity (宿主窗口骨架)             │
│  - 240×320 设计基准 + 响应式原生 DP（match_parent 铺满）      │
│  - 经典顶栏（电量/信号/时钟）+ 底部软键栏（左/中/右）        │
│  - 物理按键 DOWN/UP 严密拦截与分发                         │
│  - 实现 KeydroidxPageHost 契约                                │
└─────────────────────────────┬─────────────────────────────┘
                              │ 承载并分发按键
                              ▼
┌───────────────────────────────────────────────────────────┐
...
### 3.1 窗口骨架：`KeydroidxBaseActivity`
所有业务 Activity 必须直接继承 `io.github.cctyl.nokia.keycore.ui.KeydroidxBaseActivity`。

#### 核心职责：
- **布局装配**：子类实现 `getLayoutResId()` 提供内容区布局。
- **自动初始化**：在 `onCreate` 中自动注册 `KeydroidxClient` 配置监听器（按键/主题/字体实时同步），并递归将点阵字体应用到整棵 View 树。
- **按键中枢**：重写 `dispatchKeyEvent`，将底层硬件 KeyCode 解析为 `KeydroidxKeyAction`，并自动分发给前台的 `KeydroidxPage`。

```java
public class MyMainActivity extends KeydroidxBaseActivity {
    @Override
...
### 3.2 页面基类：`KeydroidxPageFragment` 三大模板
中间内容区的所有页面一律使用 Fragment 构建，并按场景继承以下三种基类之一：

| 页面场景 | 继承基类 | 职责与规范 |
| :--- | :--- | :--- |
| **单列列表页**<br>（设置、歌单、文件列表、历史等） | **`KeydroidxListPageFragment`** | **最常用基类**。<br>① 自动处理 UP/DOWN 首尾循环导航；<br>② 自动应用当前生态主题高亮背景（`createSelectionDrawable`）；<br>③ 自动调用防出界滚动算法；<br>④ 子类只需在 `onPageCreated` 绑定 `itemViews` 与 `listScroll`，并覆写 `onItemClicked`。 |
| **长文本 / 说明 / 表单**<br>（小说阅读、版本说明、详情） | **`KeydroidxScrollPageFragment`** | 自动查找布局中的 `ScrollView`，上下键自动触发平滑步进滚动（默认 48dp 步长）。 |
| **自定义普通页面**<br>（音乐播放器主页、表盘、九宫格） | **`KeydroidxPageFragment`** | 模板基类。固化 `getLayoutRes()` 布局膨胀、整树点阵字体渲染与生命周期守卫，子类自定义复杂焦点与视图。 |

#### `KeydroidxListPageFragment` 标准开发范例：
```java
public class SettingListFragment extends KeydroidxListPageFragment {
...
### 3.3 页面描述契约：`KeydroidxPage` 接口
`KeydroidxPageFragment` 默认实现了 `KeydroidxPage`。页面**严禁直接 `findViewById` 去修改 Activity 的顶栏或底部软键文字**，必须采用**声明式 Getter**：

```java
public interface KeydroidxPage extends KeydroidxFocusHost {
...
- ✅ **强制**：所有按键必须通过 `KeydroidxKeyBinding.resolveAction(event)` 统一解析为语义动作（`KeydroidxKeyAction`）：

```java
int action = keyBinding.resolveAction(event);
switch (action) {
    case KeydroidxKeyAction.UP:         // 向上移动焦点 / 滚动
    case KeydroidxKeyAction.DOWN:       // 向下移动焦点 / 滚动
    case KeydroidxKeyAction.LEFT:       // 向左 / 上一曲 / 步退
    case KeydroidxKeyAction.RIGHT:      // 向右 / 下一曲 / 步进
    case KeydroidxKeyAction.SELECT:     // 确定 / 进入 / 播放
    case KeydroidxKeyAction.SOFT_LEFT:  // 左软键（菜单 / 选项）
    case KeydroidxKeyAction.SOFT_RIGHT: // 右软键（返回 / 取消）
}
```
...
- ✅ **规则**：凡是在 Activity / Fragment 层消费了 `ACTION_DOWN`，**必须同步拦截消费对应的 `ACTION_UP` 与 `REPEAT`**。`KeydroidxBaseActivity` 已内置该机制。
...
- ✅ **宿主开发守则**：
  ...
  2. 页面最外层如果有包裹的 `ScrollView`，必须显式声明禁止获焦：
     ...
     ```
  3. 页面初次加载或切换后，必须主动为首个可聚焦项执行 `view.requestFocus()`（`KeydroidxListPageFragment` 与 `KeydroidxListFocusHelper` 已内置此逻辑）。
...
2. **防出界平滑滚动**：
   ...
   - ✅ **强制**使用 `KeydroidxListFocusHelper.smoothScrollToVisible(scroll, target)`，它通过递归累加祖先绝对偏移量，确保条目始终完整露在视口中央。

### 4.5 核心按键语义摘要
- **音乐库 / 列表页**：
  - `UP` / `DOWN`：上下移动条目高亮光标；
  - `SELECT` (确定)：播放选中歌曲并进入详情页；
  - `SOFT_LEFT` (左软键)：唤出标准选项菜单 (`KeydroidxOptionsDialog`)；
...
- ✅ **强制**：所有界面文字必须通过 `KeydroidxFontManager` 渲染为复古点阵字体（内置 `ArkPixel-12px` 与 `FusionPixel-12px`）。
- ⚠️ **动态 View 规则**：在代码中动态 `new TextView` 或通过 Adapter 动态 inflate 添加到容器后，必须手动调用一次：
  ```java
  KeydroidxFontManager.applyToViewTree(newView);
  ```
...
- ✅ **强制**：所有前景色、背景色、高亮色必须从 `KeydroidxTheme.ThemeDef` 或 `KeydroidxTheme.getCurrentTheme()` 中获取：
...
- **选中高亮标准实现**：
  ```java
  // 列表行选中背景（4dp 圆角矩形）
  Drawable selectDrawable = KeydroidxTheme.createSelectionDrawable(context, 4f);
...
- ✅ **强制**：统一使用 `KeydroidxIcons` 获取 Google Material Icons（内置 2500+ 图标，矢量光栅化居中，边缘极其锐利）：

```java
// 获取指定图标、指定尺寸（DP）、指定颜色的 Drawable
Drawable icon = KeydroidxIcons.createDrawable(context, KeydroidxIcons.ICON_SETTINGS, 20, Color.WHITE);
...
- UI 采用 240×320 设计基准 + 响应式原生 DP（无运行时缩放），验证时需在 240×320（`4a24ecf`）与 320×480（tcpip）两台真机上截图确认无横向溢出、无裁切错位。
...
│  - 底部弹出、透明遮罩、紧凑复古卡片底色                    │
│  - 自动绑定当前生态主题渐变顶栏与软键                     │
│  - 内置 KeydroidxDialogFocus.forceNonTouchMode 首键防吞修复   │
└───────────────────────────────────────────────────────────┘
```

### 6.1 选项菜单：`KeydroidxOptionsDialog`
用于替代原生弹出菜单/上下文菜单：
```java
new KeydroidxOptionsDialog(context, "歌曲选项")
    .addItem(1, "播放", KeydroidxIcons.createDrawable(context, KeydroidxIcons.ICON_PLAY, 18, Color.WHITE))
    .addItem(2, "添加到歌单", KeydroidxIcons.createDrawable(context, KeydroidxIcons.ICON_ADD, 18, Color.WHITE))
    .addItem(3, "删除歌曲", KeydroidxIcons.createDrawable(context, KeydroidxIcons.ICON_DELETE, 18, Color.WHITE))
...
### 6.2 确认提示框：`KeydroidxConfirmDialog`
用于替代原生确认/提示对话框：
```java
new KeydroidxConfirmDialog(context, "删除确认", "确定要删除该条记录吗？")
...
- **本地依赖**：依赖同级 `../keydroidx-core`，`settings.gradle` 通过 `includeBuild` + `dependencySubstitution` 将 `io.github.cctyl.nokia:keydroidx-key-core` 替换为本地项目。
...
- [ ] **继承合规**：所有 Activity 继承自 `KeydroidxBaseActivity`，所有内容 Fragment 继承自 `KeydroidxListPageFragment` / `KeydroidxScrollPageFragment` / `KeydroidxPageFragment`。
- [ ] **日志合规**：全工程禁止直接使用原生 `android.util.Log`，强制使用 `KeydroidxLog`（或 `import ...NLog as Log`），确保日志可分级过滤与落盘。
...
- [ ] **字体合规**：所有文字均为点阵字体，动态添加的 View 已调用 `KeydroidxFontManager.applyToViewTree(view)`。
- [ ] **配色合规**：无任何硬编码颜色，所有色值与高亮背景均来自 `KeydroidxTheme`。
- [ ] **图标合规**：无零散 PNG / XML 图标，全部使用 `KeydroidxIcons` 生成。
